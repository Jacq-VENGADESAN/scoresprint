...
function validExplanation(value: CoachExplanation) {
  return Boolean(
    ...
    && typeof value.whySelectedWasWrong === "string"
...
export async function POST(request: Request) {
  ...
  if (!/^[a-zA-Z0-9_-]{3,120}$/.test(questionCode)) {
  ...
  try {
    ...
    return NextResponse.json({ error: "L’explication n’a pas pu être générée. Réessaie plus tard.", remaining: credit.remaining }, { status: 502 });
  }
}
