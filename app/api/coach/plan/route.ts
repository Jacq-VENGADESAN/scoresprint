...
type GoalRow = { current_score: number | null; target_score: number; exam_date: string | null; daily_minutes: number; level: string | null; focus: string | null };
...
function validPlan(plan: CoachPlan) {
  return Boolean(plan && typeof plan.headline === "string" && typeof plan.diagnosis === "string" && Array.isArray(plan.prioritySkills) && Array.isArray(plan.days) && plan.days.length === 7 && plan.days.every((day) => Number.isInteger(day.minutes) && day.minutes >= 5 && day.minutes <= 120));
}
    ...
    try {
      const plan = await createStructuredResponse<CoachPlan>({
        ...
        payload: {
          ...
          available_activities: ["séance Reading adaptative", "carnet d'erreurs", "mini-examen Reading de 30 questions", "Listening partie 1", "Listening partie 2", "fiches pédagogiques"]
        },
      ...
      await supabaseRest("ai_coach_plans?on_conflict=user_id,week_start", {
        ...
        body: JSON.stringify({ user_id: user.id, week_start: weekStart, plan, model: openAiModel(), updated_at: new Date().toISOString() })
      });
      return NextResponse.json({ plan, remaining: credit.remaining, weekStart });
    } catch (error) {
      try { await refundCoachCredits(3); } catch (refundError) { console.error("Unable to refund Coach plan credits", refundError); }
      console.error("Unable to generate Coach 90 plan", error);
      return NextResponse.json({ error: "Le programme n’a pas pu être généré. Aucun crédit n’a été conservé.", remaining: credit.remaining + 3 }, { status: 502 });
    }
  } catch (error) {
    console.error("Unable to prepare Coach 90 plan", error);
